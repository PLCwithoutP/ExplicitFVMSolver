...
def findCellsOfFace(faceID, parsedOwners, parsedNbours):
    ''' This function finds the cells that face belongs to. Its value can be 1 or 2'''
    cell2 = -1
    for faces in parsedOwners:
        checkOwnerFaceID = faces[0]
        checkOwnerCellID = faces[1]
        if (faceID == checkOwnerFaceID):
            cell1 = checkOwnerCellID
    for faces in parsedNbours:
        checkNbourFaceID = faces[0]
        checkNbourCellID = faces[1]
        if (faceID == checkNbourFaceID):
            cell2 = checkNbourCellID
    return cell1, cell2
